...
printf("Ingrese el primer voto: ");//en este programa se usara los case para eleguir su voto
scanf("%d", &VOT);
while (VOT)//mientras vot sea diferente de 0 el programa ejecutara el siguiente bucle 
{
switch(VOT)
{
case 1: C1++; break;//aqui los casos y 
case 2: C2++; break;//se incrementara en 1 dependiento de la seleccion 
case 3: C3++; break;
case 4: C4++; break;
case 5: C5++; break;
default: NU++; break;//en caso de salirse de las elecciones lo contara como nulo 
}
printf("Ingrese el siguiente voto –0 para terminar–: ");
scanf("%d", &VOT);
}
SVO = C1 + C2 + C3 + C4 + C5 + NU;//aqui pues se sumara el total de votos
PO1 = ((float) C1 / SVO) * 100;//el promedio de cada eleccion 
PO2 = ((float) C2 / SVO) * 100;
PO3 = ((float) C3 / SVO) * 100;
PO4 = ((float) C4 / SVO) * 100;
PO5 = ((float) C5 / SVO) * 100;
PON = ((float) NU / SVO) * 100;
printf("\nTotal de votos: %d", SVO);//y imprimira lo siguiente 
