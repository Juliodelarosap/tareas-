...
void main(void)
{
	...
	scanf("%d %d", &DIS, &TIE);
	if ((DIS * 2  > 500) && (TIE > 10))//aqui como en casos anteriores se utiliza && 
		BIL = DIS * 2 *0.19 * 0.8;//si se cumplen las dos condiciones, realizara este calculo
	else // caso de que una no se cumpla 
		BIL = DIS * 2 * 0.19;//este seria el calculo del billete 
	printf("\n\ncosto del billete: %7.2f", BIL);//cual sea de los dos lo imprimira aqui 
}
